...
44. `vercor/setups/external/camulator_output.py` - CAMulator tensor-to-output-variable shaping, average-output path/coordinate/metadata helpers, configured output-adapter construction, period-output record delegation and latest-prediction snapshot writing through `ComponentOutputAdapter`, metadata loading, unsupported CREDIT-output option validation, and direct h5netcdf forecast-increment writing through shared VerCOR output helpers built on (25, 31)
