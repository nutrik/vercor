...
31. `vercor/output/variables.py`, `period_averages.py`, `period_files.py`, `datasets.py`, `time.py`, `netcdf.py`, and `jax_gcm.py` - shared period-output variable containers, JAX-backed sum/count period-average accumulation, shared sample/mean-output conversion helpers, period-average file write lifecycle, dataset coordinate discovery, VerCOR-calendar time encoding, direct h5netcdf averaged NetCDF writing with host conversion delegated to (10), and JAXGCM prediction-to-output adaptation built on (1, 6, 10, 25)
32. `vercor/setups/external/jax_gcm_runtime.py`, `jax_gcm_state.py`, and `jax_gcm.py` - JAXGCM private runtime-state protocol, payload/hooks/stepping, setup-state initialization and canonical `JCMState` ownership, direct runtime hook binding, and thin JCM adapter factory boundary built on (1, 3, 11, 26, 29, 30, 31, 62)
...
37. `vercor/output/veros.py`, `veros_runtime.py`, `veros_gcm_state.py`, and `veros_gcm.py` - Veros opt-in JAX-backed period-output variable/coordinate extraction and mean shaping through shared output helpers and the shared output writer, private runtime-state protocol, host-runtime stepping helper, setup-state/lifecycle ownership, named host step adapter, and thin adapter factory boundary built on (1, 6, 10, 25, 31, 33, 34, 35, 36)
...
69. `vercor/runtime/coupler_state.py` and `topology_state.py` - runtime coupler-state assembly, runtime-contract refresh, grouped exchange topology maps, surface-mask state, and topology-state contracts built on (12, 18, 19, 28, 63, 64, 66)
...
72. `vercor/runtime/compilation.py`, `run_context.py`, `cache.py`, `progress.py`, and `interrupts.py` - shared compiled-runtime callable/key aliases, frozen runtime execution context with context-derived cache-key construction, `CompiledRuntimeCache` storage/JIT policy, progress callbacks, and cancellation controller built on (24, 25, 64, 67)
73. `vercor/runtime/resources.py` - private-field per-coupler runtime resource holder, grouped replacement/accessor boundary for topology maps and refreshed contracts, compiled-runtime cache-owner delegation, and interrupts built on (63, 64, 69, 72)
...
76. `vercor/output/runtime.py` and `vercor/output/__init__.py` - runtime-view output mask selection/naming, coupler-final-output direct h5netcdf boundary through (31), and lazy top-level runtime-output reexports built on (10, 12, 31, 60, 64)
