...
37. `vercor/setups/external/veros_output.py`, `veros_runtime.py`, `veros_gcm_state.py`, and `veros_gcm.py` - Veros opt-in JAX-backed h5netcdf period-output variable/coordinate extraction and writing, private runtime-state protocol, host-runtime stepping helper, setup-state/lifecycle ownership, named host step adapter, and thin adapter factory boundary built on (1, 6, 10, 25, 31, 34, 35, 36)
...
76. `vercor/output.py` - runtime-view output mask selection/naming and coupler-final-output NetCDF output boundary built on (10, 12, 60, 64)
