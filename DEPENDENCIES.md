...
31. `vercor/output/variables.py`, `period_averages.py`, `time.py`, `netcdf.py`, and `jax_gcm.py` - shared period-output variable containers, JAX-backed sum/count period-average accumulation, VerCOR-calendar time encoding, direct h5netcdf averaged NetCDF writing with host conversion delegated to (10), and JAXGCM prediction-to-output adaptation built on (1, 6, 10, 25)
...
76. `vercor/output/runtime.py` and `vercor/output/__init__.py` - runtime-view output mask selection/naming, coupler-final-output NetCDF output boundary, and lazy top-level runtime-output reexports built on (10, 12, 60, 64)
