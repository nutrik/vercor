# -*- coding: utf-8 -*-
#
# Vercor documentation build configuration file, created by
...
project = "Vercor"
copyright = "2025-2026, Vercor, NBI/UCPH Copenhagen"
author = "The Vercor Team, NBI/UCPH Copenhagen"
...
vercor_version = tomllib.loads(pyproject_path.read_text(encoding="utf-8"))["project"][
    "version"
]
...
latex_documents = [
    (master_doc, "vercor.tex", "Vercor Documentation", "The Vercor Team", "manual"),
]
...
man_pages = [(master_doc, "Vercor", "Vercor Documentation", [author], 1)]
...
texinfo_documents = [
    (
        ...
        author,
        "Vercor",
        "The Versatile Earth system COupleR in Python",
        "Miscellaneous",
    ),
]
...
class ExecDirective(Directive):
    ...
    def run(self):
        ...
        try:
            ...
            warn_text = "Unable to execute python code at %s:%d" % (
                basename(source),
                self.lineno,
            )
...
class ClickDirective(Directive):
    ...
    option_spec = {"args": directives.unchanged}
    has_content = False
    ...
    def run(self):
        ...
        try:
            ...
            if result.exit_code != 0:
                raise RuntimeError(
                    'Command exited with non-zero exit code; output: "%s"' % text
                )
            ...
            warn_text = "Error while running command %s %s" % (
                arg,
                " ".join(map(shlex.quote, options)),
            )
            warning = self.state_machine.reporter.warning(warn_text)
            return [
                warning,
