...
and lifecycle policies.  ``CallableComponent`` and ``DataComponent`` are
convenient ways to provide that same contract.
...
field names, shapes, and dtypes at the component boundary.  A component step
returns a mapping of declared output updates, or a ``StepResult`` when it must
also replace its runtime payload.

Runtime state is immutable.  ``Coupler.initial_state()`` creates a
``RunState``; methods such as ``RunState.replace_fields`` return a new state
instead of mutating the current one.  This makes experiments reproducible and
keeps JAX transformations well defined.
...
payload returned by ``SetupResult`` and then replaced through ``StepResult``.
