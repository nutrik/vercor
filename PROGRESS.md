...
- Curated stable and advanced Python API references completed locally (2026-07-27): six explicit-autodoc pages now document only approved public owner modules and member lists, with runtime/topology extension contracts isolated under an advanced warning and two-tier API navigation. The strict build exposed obsolete NumPy/SciPy/h5netcdf autodoc mocks; because Read the Docs installs the project and core dependencies, those mocks were removed and a config regression protects real JAX imports. TDD RED was the expected missing `assembly.rst`; focused API/config checks passed 4/4 and 1/1, fast passed 741/741, and full passed 1,381/1,381 with the five known third-party warning instances. Black, flake8, and mypy checks passed.
...
candidate commit. Put detailed command evidence in the active task report and archive only durable outcomes here.
