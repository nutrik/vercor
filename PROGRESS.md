...
- Packaged setup gallery CLI Tasks 3, 5, and 6 completed locally (2026-07-30): the root `vercor --version` flag and `show-setups`, `copy-setup run_jcm_with_veros --to ~/vercor-setups/run_jcm_with_veros`, and `run --loglevel info --float-type float64 ~/vercor-setups/run_jcm_with_veros/run_jcm_with_veros.py` commands are documented. `VERCOR_SETUP_DIR` is an `os.pathsep`-separated direct-directory list with shared discovery and duplicate-name rejection; `--to` creates or reuses a directory but exclusively copies without overwrite. `run` invokes the private shell-free child with the exact `run_setup(*, loglevel, float_type)` contract, `None`/integer status behavior, lowercase `trace`/`debug`/`info`/`warning`/`error` and `float64`/`float32` choices, and `info`/`float64` defaults; gallery templates map trace-capable Coupler logging and dtype to `DTypePolicy`/`RuntimeOptions` (`trace == 5`). The added installed-artifact runner assertion/probe first ran `conda run -n scipy pytest tests/test_distribution_boundaries.py::test_built_distributions_run_external_extension_fixture_outside_checkout -q --tb=short` GREEN (1/1): prior Tasks 3/5 already shipped the runner in both artifacts, so no packaging RED was observable. The prescribed focused CLI/runner/distribution/docs command passed 129/129 (counted with the matching collection command), and `conda run -n scipy sphinx-build -W -b html docs docs/_build/html` passed. The required fast command passed its 772 selected tests (705 deselected by fresh collection); full, coverage, and release gates remain outside this task's focused evidence.
