...
- Packaged setup gallery CLI final-review fix wave completed locally (2026-07-30): all accepted runtime, copy, catalog, installed-console, help, documentation, plan, and test-fixture findings are implemented.
  - TDD RED isolated lazy adjacent import, local runner shadowing, two partial-stream failures, dotted-stem and ambiguous-alias lookup, bare `.`/`..`, and exact root help; the installed-wheel console probe separately exited 23 through the shadow runner.
  - Focused GREEN passed 17/17 plus the review regression; the final CLI/runner/distribution/docs selection passed 139/139, including a real no-dependency wheel-installed `vercor` workflow for `--version`, `show-setups`, `copy-setup --to`, and lazy external `run`.
  ...
  - Final static gates passed: Black left five changed Python files unchanged with the known Python 3.13/target-3.15 advisory; strict flake8 reported 0; focused mypy passed five files; compileall and `git diff --check` exited 0.
  - The required fast suite passed its 773/1,487 selection with 714 deselected and four known Flax/JAX warnings; active memory remains exactly 180 lines and its focused contract passed.
  - CLI docs mark the commands unreleased and absent from PyPI `0.4.3`; the corrected plan records final-review authority. No push, PR update, publication, or remote mutation occurred.
