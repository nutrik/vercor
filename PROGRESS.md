...
- Packaged setup gallery CLI final verification completed locally (2026-07-30): `vercor --version`, `show-setups`, `copy-setup --to`, and `run` support external discovery, duplicate rejection, exclusive destination copies, and the shell-free `run_setup(*, loglevel, float_type)` contract.
  - Static gates (exit 0): `conda run -n scipy black vercor tests` (245 unchanged; known Python 3.13/target-3.15 advisory) and `conda run -n scipy flake8 . --count --max-line-length=120 --statistics` (0).
  - Type, compile, and whitespace gates (exit 0): `conda run -n scipy mypy vercor tests` (245 files), `conda run -n scipy python -m compileall -q vercor tests`, and `git diff --check`.
  - Four-worker suites (exit 0): `conda run -n scipy pytest tests/ -q --fast --tb=short` (772/1,477; 705 deselected) and `conda run -n scipy pytest tests/ -q --tb=short` (1,477); warnings were four Flax/JAX deprecations, plus JCM/xarray on full.
  - Coverage (exit 0): `conda run -n scipy pytest --cov=vercor --cov-branch --cov-report=term-missing tests/ -q --tb=short` reached 91.02% (90.0% required; same five warnings).
  - Artifacts (exit 0): `conda run -n scipy python -m build` built `vercor-0.4.3.tar.gz` and `vercor-0.4.3-py3-none-any.whl`; `conda run -n scipy pytest tests/test_distribution_boundaries.py -q --tb=short` passed 26/26, including runner, all ten gallery scripts, console entry point, and isolated probe.
  - Review fixes: test-only MyPy narrowing and private `_copy_setup` retained the CLI/public-boundary contract; targeted MyPy, 13 example, 2 boundary, and 61 CLI tests passed. Detailed evidence is in the Task 7 report.
