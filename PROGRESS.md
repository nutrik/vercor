...
- Packaged setup gallery CLI final verification completed locally (2026-07-30): `vercor --version`, `show-setups`, `copy-setup run_jcm_with_veros --to ~/vercor-setups/run_jcm_with_veros`, and `run --loglevel info --float-type float64 ~/vercor-setups/run_jcm_with_veros/run_jcm_with_veros.py` are supported. `VERCOR_SETUP_DIR` remains an `os.pathsep` direct-directory list with shared discovery and duplicate rejection; `--to` creates or reuses directories and copies exclusively; `run` invokes the private shell-free child using exact `run_setup(*, loglevel, float_type)` with `None`/integer statuses, lowercase `trace`/`debug`/`info`/`warning`/`error`, `float64`/`float32`, and `info`/`float64` defaults. Fresh Black left 245 files unchanged (exit 0; known Python 3.13/target-3.15 safety-parse advisory); `conda run -n scipy flake8 . --count --max-line-length=120 --statistics` returned 0; MyPy found no issues in 245 files; compileall and `git diff --check` returned 0. The configured four-worker `conda run -n scipy pytest tests/ -q --fast --tb=short` and full `conda run -n scipy pytest tests/ -q --tb=short` both returned 0; fresh collection recorded 772/1,477 fast tests (705 deselected) and 1,477 full tests. Fast emitted four known Flax/JAX `jax.core.Effect` deprecations; full added one JCM/xarray `compat` future warning. `conda run -n scipy pytest --cov=vercor --cov-branch --cov-report=term-missing tests/ -q --tb=short` returned 0 at 91.02% total branch coverage (7,581 statements; 1,614 branches; 495 statements and 265 branches missed), exceeding 90.0%; it emitted the same five warnings. `conda run -n scipy python -m build` returned 0 and built `vercor-0.4.3.tar.gz` plus `vercor-0.4.3-py3-none-any.whl`; `conda run -n scipy pytest tests/test_distribution_boundaries.py -q --tb=short` returned 0 (26/26), confirming both artifacts contain `vercor/_setup_runner.py`, all ten gallery scripts, and `vercor=vercor.cli:cli`, plus the isolated installed-artifact copy and lightweight `run_setup` probe. Final verification exposed two test-only MyPy annotations and a non-exported `vercor.cli.copy_setup` callback; the former was narrowed/removed and the latter renamed `_copy_setup` without changing the command interface. Targeted MyPy, 13 related example tests, the 2 public-boundary regressions, and all 61 CLI tests then returned 0 before the fresh whole-suite gates.
...
- VerCOR 0.4.3 release evidence was refreshed after the README and release-runbook corrections (2026-07-25): local focused release contracts passed 30/30, fast passed 711/711 with four known Flax/JAX `jax.core.Effect` deprecation warnings, full passed 1,349/1,349 with those four plus the known JCM/xarray `compat` future warning, and branch coverage was 91.13%. [PR #14](https://github.com/nutrik/vercor/pull/14) merged the reviewed candidate after exact PR run [30168891040](https://github.com/nutrik/vercor/actions/runs/30168891040) succeeded; release commit `10717bb716bad51b94a617ae7d77a4c6e340825d` then passed exact `main` push run [30169149136](https://github.com/nutrik/vercor/actions/runs/30169149136). Annotated tag object `1bc7ac6ead5e61c77d44d64f8c408d790dc671ff` peels to that release commit. Exact tag run [30169571170](https://github.com/nutrik/vercor/actions/runs/30169571170) completed all 13 validation jobs, the PyPI upload, and PyPI verification before terminating with failure after creating zero-asset draft `359829592`; the subsequent listing-consistency race is an inference. The post-release follow-up adds bounded, state-specific visibility polling after every GitHub mutation; it retries only the exact prior valid state and fails closed on every other snapshot. The separately authorized exact-draft recovery resumed draft `359829592`, uploaded only CI wheel asset `489658421` and sdist asset `489658452`, and published [GitHub Release `v0.4.3`](https://github.com/nutrik/vercor/releases/tag/v0.4.3). Fresh exact-run, PyPI JSON, and GitHub Release downloads match the manifest: `vercor-0.4.3-py3-none-any.whl`, 213400 B, SHA-256 `76cb3d9c856037549273fa08412477d9fed730d0f94e5df265997dd4250f7582`; `vercor-0.4.3.tar.gz`, 156062 B, SHA-256 `591b86343299fd11227f7a58a501f7986977597d057dd11b8e520d6624d2fb6f`. A fresh no-cache isolated `vercor==0.4.3` installation reported metadata version 0.4.3 and imported `Clock`, `Coupler`, `Exchange`, `RectilinearGrid`, `RunState`, and `RuntimeOptions`. Final release ID `359829592` is titled `VerCOR 0.4.3`, non-draft and non-prerelease, with exactly those two assets. Historical local and remote tag identities remain unchanged: `v0.4.0` object `8b67b06773810b5b669426939d746834701a65e3`; `v0.4.1` object `f4fc2a5661b73e549e6076cf14431242984abc50`, peeled `e0748ee5d31e40cf08a2066013981f35c91bea0f`; `v0.4.2` object `5175ad1a9134b1694536251585e2deb4116379c5`, peeled `180c08646e72b85186810142010ae699396976fc`.
...
strict flake8, mypy, compileall, fast and full pytest, branch coverage, build, installed wheel/sdist/plugin smokes, and `git diff --check` before a release candidate commit. Put detailed command evidence in the active task report and archive only durable outcomes here.
