...
- Packaged setup gallery CLI final-review and installed-wheel CI fix wave completed locally (2026-07-30): all accepted runtime, copy, catalog, installed-console, help, documentation, plan, and test-fixture findings are implemented.
  - TDD RED isolated the accepted review findings; the wheel-CI regression then reproduced pip accepting an inherited same-version installation without creating the venv-local `vercor` entry point.
  ...
  - The required fast suite passed its 773/1,487 selection with 714 deselected; full and coverage passed 1,487/1,487 at 91.09%, with four known Flax/JAX warnings plus one known JCM/xarray warning in full runs; active memory remains exactly 180 lines.
  - CLI docs mark the commands unreleased and absent from PyPI `0.4.3`; the corrected plan records final-review authority. No publication or remote release mutation occurred.
