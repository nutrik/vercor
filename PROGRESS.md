...
- Packaged setup gallery CLI completed locally (2026-07-29): the former top-level `examples/` package is removed and its ten runnable scripts now ship under `vercor.setups.gallery`. The installed `vercor copy-setup NAME` accepts a stem or `.py` filename, rejects indirect/non-Python/private-resource names, creates the current-directory destination exclusively without overwriting collisions, and removes only a newly created partial file after copy failure. `vercor run FILE.py` invokes the local file with `sys.executable` without a shell and preserves its exit status. README/Sphinx, release/development, design/dependency, changelog, public-owner, CI, wheel, and sdist contracts describe and protect the workflow; gallery resources are explicitly omitted from executable-library coverage. Focused public-boundary/metadata verification passed 83/83; the private-resource review regression recorded 2/2 RED then 2/2 GREEN, and the complete CLI suite passed 19/19. Black left 243 files unchanged with its Python 3.13/configured-target-Python-3.15 safety-parse advisory, flake8 reported 0, mypy passed 243 source files, compileall and whitespace checks succeeded. At fix commit `738619b`, configured four-worker xdist verification passed: fast 763/763 with four warnings in 32.46s; full 1,419/1,419 with five warnings in 68.91s; and branch coverage 1,419/1,419 at 91.09% across 7,457 statements and 1,580 branches with five warnings in 85.98s. A fresh isolated build produced `vercor-0.4.3-py3-none-any.whl` and `vercor-0.4.3.tar.gz`; the serial distribution boundary passed 26/26, including all ten gallery resources, `vercor=vercor.cli:cli` metadata, and an outside-checkout installed-resource copy probe.
