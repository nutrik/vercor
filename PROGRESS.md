...
- Packaged setup gallery CLI completed locally (2026-07-29): the former top-level `examples/` package is removed and its ten runnable scripts now ship under `vercor.setups.gallery`. The installed `vercor copy-setup NAME` accepts a stem or `.py` filename, rejects indirect/non-Python/private-resource names, creates the current-directory destination exclusively without overwriting collisions, and removes only a newly created partial file after copy failure. `vercor run FILE.py` invokes the local file with `sys.executable` without a shell and preserves its exit status. README/Sphinx, release/development, design/dependency, changelog, public-owner, CI, wheel, and sdist contracts describe and protect the workflow; gallery resources are explicitly omitted from executable-library coverage. Focused public-boundary/metadata verification passed 83/83; Black left 243 files unchanged with its Python 3.13/configured-target-Python-3.15 safety-parse advisory, flake8 reported 0, mypy passed 243 source files, compileall and whitespace checks succeeded. Fresh serial fast passed 761/761 with 656 deselected and one Flax/JAX `jax.core.Effect` deprecation warning; fresh serial full passed 1,417/1,417 with that warning plus one JCM/xarray merge-default future warning. Branch coverage passed 1,417/1,417 at 91.11% across 7,455 statements and 1,578 branches. A fresh isolated build produced `vercor-0.4.3-py3-none-any.whl` and `vercor-0.4.3.tar.gz`; the serial distribution boundary passed 26/26, including all ten gallery resources, `vercor=vercor.cli:cli` metadata, and an outside-checkout installed-resource copy probe. The configured four-worker xdist policy could not start workers in this agent environment, so the recorded final suites use explicit `-n0`; hosted/configured parallel verification remains separate.
