...
- Packaged setup gallery CLI Tasks 3 and 5 completed locally (2026-07-30): `vercor run` now executes an explicit keyword-only `run_setup(*, loglevel, float_type)` contract through a private, shell-free child module using `sys.executable`; it preserves local setup imports, ignores a setup's main guard, maps `None` to success, propagates integer statuses, and rejects malformed contracts/statuses. All ten bundled modules expose that contract; the seven former guarded scripts map CLI precision to `DTypePolicy`/`RuntimeOptions` and log level to `Coupler` while retaining direct-run defaults. The Click command adds `-v/--loglevel` (`trace`, `debug`, `info`, `warning`, `error`; default `info`) and `--float-type` (`float64`, `float32`; default `float64`), with real-child tests covering all choices, defaults, local/import isolation, option-like filenames, exceptions, and statuses. TDD RED was the absent runner import, followed by 17 CLI behavior failures that proved the direct-execution path lacked contract invocation/options and 7 missing gallery contracts; GREEN was 57 focused runner/CLI tests plus the live all-module gallery contract. The first required fast run correctly detected the new private module missing from the architecture inventory; documenting it restored the targeted inventory test and fast suite to exit 0, with four known Flax/JAX deprecation warnings. The child-test fixture prepends this worktree to `PYTHONPATH` because the active interpreter's editable installation targets the separate canonical checkout; production execution remains the approved `sys.executable -m vercor._setup_runner` invocation.
